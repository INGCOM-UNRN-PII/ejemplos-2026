...
Tenemos una clase `Empleado` con un código de tipo (ENGINEER, SALESMAN, MANAGER) y un método `calcularSalario()` que usa
un `switch`. Cada vez que agregamos un tipo de empleado, debemos modificar el `switch`.
